...
export function ChaosToScout() {
  const [stage, setStage] = useState<'chaos' | 'system'>('chaos');
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (isNext) {
        ...
        // If 'system', we let the event propagate to page.tsx to switch to the next slide
      } else if (isPrev) {
        if (stage === 'system') {
          e.preventDefault();
          e.stopImmediatePropagation();
    ...
    <div className="relative w-full h-screen flex flex-col items-center justify-center overflow-hidden bg-[#030712] px-6">
      ...
      <div className="relative w-full max-w-5xl h-[500px] flex items-center justify-center">
        ...
        <motion.div
          ...
          initial={false}
          animate={{ opacity: stage === 'chaos' ? 1 : 0, scale: stage === 'chaos' ? 1 : 0.8, filter: stage === 'chaos' ? 'blur(0px)' : 'blur(10px)' }}
          transition={{ duration: 1.2, ease: [0.22, 1, 0.36, 1] }}
          style={{ pointerEvents: stage === 'chaos' ? 'auto' : 'none' }}
        >
          <div className="flex items-center justify-center gap-12 w-full">
            <div className="text-3xl font-mono text-white/50 tracking-widest uppercase">URL</div>
            ...
            <div className="text-3xl font-mono text-red-500/50 tracking-widest uppercase">Broken</div>
          </div>
        </motion.div>
        ...
        <motion.div
          className="absolute inset-0 flex items-center justify-center"
          initial={false}
          animate={{ opacity: stage === 'system' ? 1 : 0, scale: stage === 'system' ? 1 : 1.1, filter: stage === 'system' ? 'blur(0px)' : 'blur(10px)' }}
          transition={{ duration: 1.2, ease: [0.22, 1, 0.36, 1], delay: stage === 'system' ? 0.3 : 0 }}
          style={{ pointerEvents: stage === 'system' ? 'auto' : 'none' }}
        >
          <div className="flex items-center justify-center gap-16 w-full">
            <div className="text-3xl font-mono text-white tracking-widest uppercase">URL</div>
            
            <div className="flex items-center gap-2">
              <div className="w-16 h-0.5 bg-gradient-to-r from-white/20 to-violet-500/50 relative overflow-hidden">
                <motion.div className="absolute inset-0 bg-violet-400" animate={{ x: ['-100%', '100%'] }} transition={{ duration: 1.5, repeat: Infinity }} />
              </div>
            ...
            <motion.div
              ...
              animate={{ boxShadow: ['0 0 40px rgba(139,92,246,0.2)', '0 0 80px rgba(139,92,246,0.4)', '0 0 40px rgba(139,92,246,0.2)'] }}
            ...
            <div className="flex items-center gap-2">
              <div className="w-16 h-0.5 bg-gradient-to-r from-violet-500/50 to-emerald-500/50 relative overflow-hidden">
                <motion.div className="absolute inset-0 bg-emerald-400" animate={{ x: ['-100%', '100%'] }} transition={{ duration: 1.5, repeat: Infinity, delay: 0.5 }} />
              </div>
